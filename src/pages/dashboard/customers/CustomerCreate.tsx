...
export const CustomerCreate: React.FC = () => {
  ...
  const validate = (): string | null => {
    if (!name.trim()) return 'Customer Name wajib diisi';
    if (!mobilePhone.trim()) return 'Nomor HP wajib diisi';
    if (!address.trim()) return 'Customer Address wajib diisi';
    if (!city) return 'City wajib dipilih';
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post<unknown>('/services/customers/create', payload, token ? { Authorization: token } : undefined);
      if (res.status === 'success') {
        await Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Customer berhasil ditambahkan.' });
        navigate(`${basePrefix}/customers`);
      }
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Customer Baru</h1>
          <p className="text-gray-600 dark:text-gray-300 mt-1">Lengkapi data customer</p>
        </div>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={() => navigate(`${basePrefix}/customers`)}>
            Batal
          </Button>
